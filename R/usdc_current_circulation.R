...
ethereum <- function() {
  ...
  httr::content(r, as = 'text', encoding = 'UTF-8') %>% jsonlite::fromJSON() %>% .$data
...
algorand <- function() {
  ...
  stopifnot(status_code(r) == 200)
  httr::content(r, as = 'text', encoding = 'UTF-8') %>% jsonlite::fromJSON()
...
stellar <- function() {
  ...
  stopifnot(status_code(r) == 200)
  httr::content(r, as = 'text', encoding = 'UTF-8') %>% jsonlite::fromJSON() %>% .$`_embedded` %>% .$records
